...
from claw.api.client import (
    ProviderClient,
    max_tokens_for_model,
    resolve_model_alias,
)
from claw.cli.render import CLAW_THEME, TerminalRenderer
...
from claw.commands.registry import get_command_registry
from claw.commands.handlers.agents import handle_agents_command
from claw.commands.handlers.mcp import handle_mcp_command
from claw.commands.handlers.plugins import handle_plugins_command
from claw.commands.handlers.skills import handle_skills_command
...
from claw.runtime.conversation import ConversationRuntime, TurnSummary
...
from claw.runtime.prompt import SystemPromptBuilder
from claw.runtime.sandbox import detect_sandbox
...
from claw.runtime.usage import UsageTracker, format_usd
from claw.tools.registry import BuiltinToolExecutor, get_tool_registry
...
HISTORY_FILE = ".claw/repl_history"
MAX_SESSION_LIST = 20
...
def _export_transcript(session: Session, output_path: Path) -> None:
    ...
    lines: list[str] = []
    lines.append(f"# Claw Code Session Transcript")
    lines.append(f"")
...
async def _handle_slash_command(
    ...
    # --- Version ---
    if cmd == "version":
        return f"claw-code {__version__}"
...
def _handle_session_command(args: str, session: Session) -> str:
    ...
    if action == "fork":
        from claw.runtime.session import SessionFork
...
async def run_repl(
    ...
    try:
        while True:
            # Read input
            try:
                user_input = await prompt_session.prompt_async("claw> ")
            except (EOFError, KeyboardInterrupt):
                console.print("\n[dim]Goodbye![/dim]")
                break
...
def _run_logout(provider_name: str = "anthropic") -> int:
    ...
    console.print("[bold]Claw Code Logout[/bold]\n")
    existing = load_oauth_credentials(provider_name)
    if existing is None:
...
def cli(
    ...
    if version:
        click.echo(f"claw-code {__version__}")
        return
...
def status(ctx: click.Context) -> None:
    ...
    console.print(f"[bold]Claw Code[/bold] v{__version__}")
...
def session_cmd(action: str, args: tuple[str, ...]) -> None:
    ...
    if action == "list":
        ...
        if not args:
            console.print("[yellow]Usage: claw session show <session_id>[/yellow]")
            return
        ...
        if not args:
            console.print("[yellow]Usage: claw session delete <session_id>[/yellow]")
            return
        path = _resolve_session(args[0])
        if path is None:
            console.print(f"[red]Session not found: {args[0]}[/red]")
