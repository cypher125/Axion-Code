...
CLAW_THEME = Theme({
    "claw.prompt": "bold cyan",
    "claw.assistant": "white",
    "claw.tool_name": "bold cyan",
    "claw.tool_border": "dim",
    "claw.tool_input": "dim",
    "claw.tool_output": "green",
    "claw.tool_success": "bold green",
    "claw.tool_error": "bold red",
    "claw.error": "bold red",
    "claw.warning": "yellow",
    "claw.info": "dim",
    "claw.cost": "dim cyan",
    "claw.status": "dim",
    "claw.heading": "bold cyan",
    "claw.code": "green",
    "claw.link": "blue underline",
    "claw.thinking": "dim italic",
})
...
class TerminalRenderer:
    ...
    def render_tool_call(self, tool_name: str, tool_input: str) -> None:
        ...
        self.console.print(f"[claw.tool_border]{formatted}[/claw.tool_border]")
    ...
    def render_tool_result(self, tool_name: str, output: str, is_error: bool = False) -> None:
        ...
        style = "claw.tool_error" if is_error else "claw.tool_border"
    ...
    def render_tool_use_simple(self, tool_name: str, tool_input: str) -> None:
        ...
        self.console.print(f"[claw.tool_name]⚡ {tool_name}[/claw.tool_name]")
        if tool_input:
            display = tool_input[:500] + "..." if len(tool_input) > 500 else tool_input
            self.console.print(f"  [claw.tool_input]{display}[/claw.tool_input]")

    def render_tool_result_simple(self, output: str, is_error: bool = False) -> None:
        """Render a simple tool result (no box)."""
        if is_error:
            self.console.print(f"[claw.tool_error]✗ {output}[/claw.tool_error]")
        else:
            display = output[:1000] + "\n... (truncated)" if len(output) > 1000 else output
            self.console.print(f"[claw.tool_output]{display}[/claw.tool_output]")

    def render_thinking(self, text: str) -> None:
        """Render collapsed thinking output."""
        preview = text[:100] + "..." if len(text) > 100 else text
        self.console.print(f"[claw.thinking]💭 Thinking: {preview}[/claw.thinking]")

    def render_error(self, message: str) -> None:
        """Render an error message."""
        self.console.print(f"[claw.error]Error: {message}[/claw.error]")
    ...
    def render_context_window_error(
        ...
        lines = [
            "[claw.error]Context window blocked[/claw.error]",
            f"  Model: {model}",
    ...
    def render_warning(self, message: str) -> None:
        ...
        self.console.print(f"[claw.warning]Warning: {message}[/claw.warning]")

    def render_info(self, message: str) -> None:
        """Render an info message."""
        self.console.print(f"[claw.info]{message}[/claw.info]")

    def render_cost(self, cost_line: str) -> None:
        """Render cost information."""
        self.console.print(f"[claw.cost]{cost_line}[/claw.cost]")
